...
if __name__ == '__main__':
    ...
    print(" ================== Evaluating ==================  ")
    net.eval()
    flops, params = profile(net, inputs=(torch.rand((1, 2, 1024), device=device),), verbose=False)
        ...
        if epoch == 0:
            ...
            for step, (batch_x, batch_y) in enumerate(train_loader_tqdm):
                ...
                if torch.rand(1, requires_grad=False) > 0.7 and step != batch_length - 1:
                    sig_length = torch.randint(512, 1024, (1, 1), requires_grad=False).item()
                    sig_start = torch.randint(0, 1024-sig_length, (1, 1), requires_grad=False).item()
